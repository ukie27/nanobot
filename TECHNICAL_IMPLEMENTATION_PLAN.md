...
在现有包内先建立 `nanobot/career` 的清晰边界。通用 Agent 模块保持可用，Career 代码通过 Port 使用其能力。只有确认发生职责冲突、循环依赖或安全问题时才移动或重写旧模块；删除只针对确认冗余且没有扩展价值的内容。
...
agent adapter → application ports + existing nanobot runtime
...
- Domain 导入 FastAPI、SQLAlchemy、OpenCLI、IMAP 或 nanobot Agent。
...
```json
{
  "type": "https://nanobot-career.local/problems/version-conflict",
...
nanobot-career serve
nanobot-career doctor
nanobot-career status
nanobot-career db migrate
nanobot-career db backup
nanobot-career db restore <backup>
nanobot-career connectors list
nanobot-career connectors health <id>
nanobot-career connectors sync <id>
nanobot-career jobs list
nanobot-career jobs retry <id>
nanobot-career export
nanobot-career delete-all-data
```
...
本方案把 nanobot 的 Agent 能力降到正确的位置：它是受控推理引擎，不是业务数据库和状态机。Career Domain 成为系统事实源，Application Service 负责用例编排，Connector 只采集外部事件，Review Task 承接所有需要用户裁决的变化。
