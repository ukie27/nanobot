# Nanobot Career 产品功能设计与落地方案

> 文档状态：Product redesign baseline  
> 日期：2026-07-26  
> 适用范围：`nanobot-career` 本地单用户求职工作台  
> 依据：`CODEX_REQUIREMENTS_ANALYSIS.md`、`CAREER_REQUIREMENTS_ANALYSIS.md`、`TECHNICAL_IMPLEMENTATION_PLAN.md`、`DEVELOPMENT_PLAN.md`、当前 Part 0–9 实现及本轮产品讨论

## 1. 结论

Nanobot Career 不应被设计成若干独立 AI 功能的集合，也不应以聊天记录或通用 Agent Memory 作为核心状态。产品的核心是一套本地、可追溯、持续演进的求职业务系统：
...
职业档案必须保存为结构化、版本化的业务数据，不写入 Nanobot 通用 `memory/MEMORY.md`，也不依赖聊天历史。建议分为五层：
...
2. [已完成] 接入共享 AgentRunner 的 tool-free Nanobot Task Agent；不加载会话、SOUL、工作区记忆或 Skills。
...
- `TECHNICAL_IMPLEMENTATION_PLAN.md` 继续描述通用工程架构，但需要根据 Opportunity、业务记忆和 AgentTaskPort 补充修订。
- `DEVELOPMENT_PLAN.md` 需要从原 Part 0–9 完成状态转为本文件 Phase A–E 的产品改进计划。
- 每个 Phase 开发前另建短期实施文档，冻结 Schema、迁移、API、页面和验收用例。

## 23. Nanobot Runtime 适配与改造

### 23.1 总体结论

Nanobot 在本项目中是可修改的源码基础，不是需要保留兼容性的独立产品。目标是形成一套 **Career-only Runtime**：
...
判断标准不是“它原来属于 Nanobot”，而是：是否符合 Career 业务模型、可靠性、安全性和长期维护要求。
...
现有 `nanobot/agent/memory.py` 不再承担 Career 业务知识。可以重写为基于业务知识系统的检索/上下文接口，或在确认没有独立对话记忆需求后移除其中的 Markdown consolidation；具体代码处置不影响 AgentLoop 继续存在。
...
### 23.11 Nanobot 模块处置矩阵

| Nanobot 模块 | 目标处置 | Career 用法 |
|---|---|---|
...
Nanobot Runtime 改造应嵌入产品 Phase，而不是单独大重写：
...
旧 Career Store 和粗糙 Career Tool 不需要数据迁移；移除启动引用和旧测试后可直接删除。Nanobot AgentLoop、Cron、Channels、Tools 等通用能力则按上述方向保留和改造，不纳入无差别清理范围。
