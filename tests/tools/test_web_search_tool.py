...
from career_console.runtime.agent.tools.web import WebSearchTool
from career_console.runtime.config.schema import WebSearchConfig
...
async def test_brave_search(monkeypatch):
    async def mock_get(self, url, **kw):
        ...
        return _response(json={
            "web": {"results": [{"title": "CareerConsole", "url": "https://example.com", "description": "AI assistant"}]}
        })

    monkeypatch.setattr(httpx.AsyncClient, "get", mock_get)
    tool = _tool(provider="brave", api_key="brave-key")
    result = await tool.execute(query="careerconsole", count=1)
    assert "CareerConsole" in result
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("career_console.runtime.agent.tools.web.DDGS", MockDDGS, raising=False)
    import career_console.runtime.agent.tools.web as web_mod
