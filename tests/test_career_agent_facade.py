"""Tests for the CareerAgent programmatic facade."""
...
def test_from_config_missing_file():
    with pytest.raises(FileNotFoundError):
        CareerAgent.from_config("/nonexistent/config.json")


def test_from_config_creates_instance(tmp_path):
    config_path = _write_config(tmp_path)
    bot = CareerAgent.from_config(config_path, workspace=tmp_path)
    assert bot._loop is not None
    assert bot._loop.workspace == tmp_path


def test_from_config_default_path(tmp_path: Path):
    from career_console.runtime.config.schema import Config

    with patch("career_console.runtime.config.loader.load_config") as mock_load, \
         patch("career_console.runtime.facade._make_provider") as mock_prov, \
         patch(
             "career_console.runtime.config.paths.default_workspace_path",
...
async def test_run_hooks_restored_on_error(tmp_path):
    ...
    from career_console.runtime.agent.hook import AgentHook
...
def test_workspace_override(tmp_path):
    ...
    bot = CareerAgent.from_config(config_path, workspace=custom_ws)
    assert bot._loop.workspace == custom_ws


def test_sdk_make_provider_uses_github_copilot_backend():
    from career_console.runtime.config.schema import Config
    from career_console.runtime.facade import _make_provider
    ...
    with patch("career_console.runtime.providers.openai_compat_provider.AsyncOpenAI"):
...
def test_import_from_top_level():
    from career_console.runtime import CareerAgent as N
    from career_console.runtime import RunResult as R
    assert N is CareerAgent
