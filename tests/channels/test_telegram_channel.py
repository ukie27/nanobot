...
from career_console.runtime.channels.telegram import (
...
class _FakeBot:
    ...
    async def get_me(self):
        self.get_me_calls += 1
        return SimpleNamespace(id=999, username="careerconsole_test")
...
async def test_start_creates_separate_pools_with_proxy(monkeypatch) -> None:
    ...
    monkeypatch.setattr("career_console.runtime.channels.telegram.HTTPXRequest", _FakeHTTPXRequest)
    monkeypatch.setattr(
        "career_console.runtime.channels.telegram.Application",
...
async def test_send_text_retries_on_timeout() -> None:
    ...
    import career_console.runtime.channels.telegram as tg_mod
...
async def test_on_error_logs_network_issues_as_warning(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "career_console.runtime.channels.telegram.logger.warning",
        lambda message, error: recorded.append(("warning", message.format(error))),
    )
    monkeypatch.setattr(
        "career_console.runtime.channels.telegram.logger.error",
...
async def test_send_remote_media_url_after_security_validation(monkeypatch) -> None:
    ...
    channel._app = _FakeApp(lambda: None)
    monkeypatch.setattr("career_console.runtime.channels.telegram.validate_url_target", lambda url: (True, ""))
...
async def test_send_blocks_unsafe_remote_media_url(monkeypatch) -> None:
    ...
    channel._app = _FakeApp(lambda: None)
    monkeypatch.setattr(
        "career_console.runtime.channels.telegram.validate_url_target",
...
async def test_group_policy_mention_accepts_text_mention_and_caches_bot_identity() -> None:
    ...
    await channel._on_message(_make_telegram_update(text="@careerconsole_test hi", entities=[mention]), None)
    await channel._on_message(_make_telegram_update(text="@careerconsole_test again", entities=[mention]), None)
...
async def test_group_policy_mention_accepts_caption_mention() -> None:
    ...
    await channel._on_message(
        _make_telegram_update(caption="@careerconsole_test photo", caption_entities=[mention]),
        None,
    )

    assert len(handled) == 1
    assert handled[0]["content"] == "@careerconsole_test photo"
...
async def test_download_message_media_returns_path_when_download_succeeds(
    ...
    monkeypatch.setattr(
        "career_console.runtime.channels.telegram.get_media_dir",
