...
from nanobot.career.domain.connectors import next_scan_time
...
class SqlAlchemyConnectorGateway:
    ...
    def list_runs(self, *, limit: int = 20) -> list[dict[str, Any]]:
        with self._session_factory() as session:
            rows = session.scalars(
                select(SyncRunModel)
                .where(SyncRunModel.connector_id == BOSS_CONNECTOR_ID)
    ...
    def start_run(self, *, trigger_type: str) -> dict[str, Any]:
        ...
        with self._session_factory() as session:
            ...
            acquired = session.execute(
                ...
                .where(
                    ConnectorConfigModel.id == BOSS_CONNECTOR_ID,
                    or_(
            ...
            row = SyncRunModel(
                id=run_id,
                connector_id=BOSS_CONNECTOR_ID,
            ...
            try:
                ...
                existing = session.scalar(
                    select(SourceEventModel).where(
                        SourceEventModel.connector_id == BOSS_CONNECTOR_ID,
    ...
    def due(self) -> bool:
        config = self.get_or_create_boss()
