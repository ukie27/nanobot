...
class SqlAlchemyProfileMemoryGateway:
        ...
        with self._session_factory() as session:
            ...
            for insight in result.insights:
                row = ProfileInsightProposalModel(
                    ...
                    confidence=insight.confidence, source="nanobot_profile_insight",
    ...
    def _ensure_profile(session: Session, now: datetime) -> None:
        if session.get(CandidateProfileModel, PROFILE_ID) is None:
            session.add(CandidateProfileModel(
                id=PROFILE_ID, display_name=None, timezone="Asia/Shanghai", version=1,
                created_at=now, updated_at=now,
            ))
            session.flush()
