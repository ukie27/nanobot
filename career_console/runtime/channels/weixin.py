...
from career_console.runtime.config.paths import get_media_dir, get_runtime_subdir
from career_console.runtime.config.schema import Base
from career_console.runtime.utils.helpers import split_message
...
class WeixinConfig(Base):
    ...
    state_dir: str = ""  # Default: active CareerConsole workspace runtime/weixin/
...
class WeixinChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            if not await self._qr_login():
                logger.error("WeChat login failed. Configure WeChat authentication in CareerConsole settings.")
                self._running = False
                return
    ...
    async def _send_text(
        ...
        client_id = f"career-console-{uuid.uuid4().hex[:12]}"
