...
- Publish to CareerAgent bus via BaseChannel._handle_message()
...
from career_console.runtime.config.schema import Base
from career_console.runtime.security.network import validate_url_target

try:
    from career_console.runtime.config.paths import get_media_dir
...
def _make_bot_class(channel: QQChannel) -> type[botpy.Client]:
    ...
    class _Bot(botpy.Client):
        def __init__(self):
            # Disable botpy's file log — CareerConsole uses loguru; default "botpy.log" fails on read-only fs
...
class QQConfig(Base):
    ...
    # Optional: directory to save inbound attachments. If empty, use CareerConsole get_media_dir("qq").
    media_dir: str = ""
    ...
    def _init_media_root(self) -> Path:
        ...
        if self.config.media_dir:
            ...
            try:
                ...
                root = Path.cwd() / ".career-console" / "media" / "qq"
        else:
            root = Path.cwd() / ".career-console" / "media" / "qq"
