...
class BackgroundJobService:
    ...
    def claim_next(
        ...
        now = now or datetime.now(UTC)
        with self._session_factory() as session:
            candidate = select(BackgroundJobModel.id).where(
            ...
            if job_types:
                candidate = candidate.where(BackgroundJobModel.job_type.in_(job_types))
            ...
            claimed_id = session.scalar(
                update(BackgroundJobModel)
                .where(
                    BackgroundJobModel.id == candidate.scalar_subquery(),
                    BackgroundJobModel.status == "pending",
                    BackgroundJobModel.run_after <= now,
                    BackgroundJobModel.attempt_count < BackgroundJobModel.max_attempts,
                )
            ...
            if claimed_id is None:
                ...
                return None
            session.commit()
            row = session.get(BackgroundJobModel, claimed_id)
            if row is None:  # Defensive: the queue does not currently delete rows.
                raise LookupError("Claimed background job was not found.")
