...
def run_due(body: RunDueRequest, request: Request) -> dict:
    run = request.app.state.scheduler_runtime.run_once(
        trigger_type="manual", now=body.now
    )
    counters = run["counters"]
    return {
        **{key: counters[key] for key in (
            "schedules_processed", "reminders_triggered", "outbox_dispatched",
            "leases_recovered", "connector_runs_processed",
        )},
        "connector_error_code": run["error_codes"][0] if run["error_codes"] else None,
    }


@scheduler_router.get("/configuration")
def scheduler_configuration(request: Request) -> dict:
    return request.app.state.scheduler_runtime.configuration()


@scheduler_router.put("/configuration")
def configure_scheduler(body: SchedulerUpdateRequest, request: Request) -> dict:
    try:
        return request.app.state.scheduler_runtime.configure(
            body.expected_revision, body.scheduler.model_dump()
        )
    except ConfigurationRevisionConflictError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc


@scheduler_router.get("/runs")
def scheduler_runs(request: Request, limit: int = Query(50, ge=1, le=200)) -> dict:
    return request.app.state.scheduler_runtime.list_runs(limit)
