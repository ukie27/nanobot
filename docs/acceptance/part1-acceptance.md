...
- 单条确认、编辑、拒绝以及原子批量确认。
- 本地事实提取器，以及无工具权限的 CareerConsole Task Agent 提取适配器。
- Part 0 数据库升级前自动备份。
...
如果现有 CareerConsole 配置已经设置可用模型，可以在启动服务前执行：

```powershell
$env:CAREER_CONSOLE_FACT_EXTRACTOR_MODE = 'agent'
.\.venv\Scripts\python.exe -m career_console serve
```
...
```powershell
Remove-Item Env:CAREER_CONSOLE_FACT_EXTRACTOR_MODE
```
