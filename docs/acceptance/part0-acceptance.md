...
- `career-console` 独立产品入口，不影响原有 Agent CLI。
...
```powershell
.\.venv\Scripts\python.exe -m career_console serve --port 8766
```
...
```powershell
.\.venv\Scripts\python.exe -m career_console db backup
```
