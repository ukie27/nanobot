...
Use CareerConsole programmatically — load config, run the agent, get results.

## Quick Start

```python
import asyncio
from career_console.runtime import CareerAgent
...
### `CareerAgent.from_config(config_path?, *, workspace?)`

Create a `CareerAgent` from an internal Runtime config file.

| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `config_path` | `str \| Path \| None` | `None` | Explicit Runtime JSON path. Without it, use `<active-workspace>/runtime/runtime.json`. |
...
```python
from career_console.runtime.agent import AgentHook, AgentHookContext
...
async def main():
    bot = CareerAgent.from_config(workspace="/my/project")
