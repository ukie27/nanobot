...
2. **个人档案必须可信。** Agent 结果只有通过确定性结构、来源、敏感信息、重复和事实边界校验后才能以 `confirmed` 写入；校验失败不得进入正式材料或长期策略。
...
| 我的简历 | 已实现 | 管理可复用基础/方向简历；按名称和用户要求调用 Agent，基于已确认事实生成候选；确认后建立正式版本；支持编辑、复核、定稿、PDF 导出和版本历史 | 不在此页面选择岗位或生成岗位定制版本；DOCX 正式导出不是当前保证能力 |
| 岗位申请材料 | 已实现 | 从具体岗位选择已有简历或新建岗位简历，结合已确认方向和岗位要求生成 Agent 候选，确认后形成岗位材料 | 仅在具体岗位上下文提供，不替代通用简历库 |
...
1. 进入“我的简历”查看基础简历和方向简历版本。
2. 填写简历名称和生成要求。
3. `standalone_resume_drafting` Agent 只读取已确认职业事实，输出结构化简历候选。
4. 核对正文和事实依据。确认前不会创建正式简历。
5. 确认候选后建立基础简历及首个版本。
6. 在简历详情中编辑新版本、复核、定稿并导出 PDF。

生成岗位申请材料：

1. 打开具体岗位，完成岗位分析并确认简历方向。
2. 从岗位详情进入“准备申请材料”。
3. 选择一份已有简历，或填写新简历名称。
4. Agent 结合已确认事实、方向和当前岗位要求生成岗位定制候选。
5. 核对每个内容块引用的事实和岗位要求，通过 Reviewer 检查真实性、覆盖和风险。
6. 确认后创建岗位材料，编辑、复核、定稿并导出 PDF。

岗位定制交互只存在于具体岗位上下文。“我的简历”会收纳正式的可复用版本，也可接收由岗位材料另存形成的基础或方向简历。Final 只表示材料定稿，不表示已经投递。
...
| 个人资料 > 我的简历 | 管理可复用简历版本，按名称和要求生成 Agent 候选，确认后编辑、复核、定稿和导出 |
| 目标岗位 > 准备申请材料 | 为当前岗位选择已有简历或创建岗位定制简历 |
| 导入资料 | 导入职业资料并启动事实提取 |
...
CareerConsole 已具备从个人档案、岗位发现、具体 JD、匹配推荐、材料生成，到申请冻结、邮件跟进、任务、手工面试纪要和持续职业诊断的本地业务闭环。当前主要交付缺口集中在最终 Windows 产品化安装和 Launcher，而不是核心业务对象缺失。
