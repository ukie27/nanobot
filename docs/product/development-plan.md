# CareerConsole 分阶段开发方案

> 文档状态：执行计划初稿<br>
> 依据：[需求分析](../archive/career-requirements-analysis.md)与[技术实现方案](../architecture/technical-implementation-plan.md)<br>
> 日期：2026-07-22<br>
...
6. 当前 CareerConsole 框架按影响处理：有用且无冲突的能力保留，必要部分修改，确认冗余后再删除。
...
- 锁定 Python 和前端依赖。
- 配置 Windows、Linux CI。
- 保留现有 CareerConsole Runtime，建立 `career_console` 分层目录。
...
- `career-console serve` 或最终确认的等价命令。
- `career-console doctor`。
- `career-console db migrate`。
- `career-console db backup`。
...
- 性能、稳定性和长时间运行测试。
- 安全检查和依赖漏洞扫描。
- 根据实际使用评估现有 CareerConsole 冗余模块；无影响且有扩展价值的保留，确认负担项再删除。
...
4. 以现有 `career_console/runtime/` 为源码基础建设 Career-only Runtime；允许重写和删除不符合产品目标的 Agent 框架模块。
