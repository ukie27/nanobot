...
export function JobDetailPage() {
  ...
  return <><header className="page-header job-detail-header"><div><p className="eyebrow"><Link to="/job-posts">岗位池</Link> / {job.company}</p><h1>{job.title}</h1><p>{job.location ?? "地点待确认"} · {job.work_mode ?? job.employment_type ?? "类型待确认"}{job.deadline_at ? ` · 截止 ${formatChinaTime(job.deadline_at)}（北京时间）` : ""}</p></div><div className="header-actions"><Link className="download-button" to={`/materials?jobId=${job.id}`}>生成申请材料</Link><button onClick={() => rerun.mutate()} disabled={rerun.isPending}>重新分析</button></div></header>
    ...
    <section className="detail-grid"><details className="panel"><summary>查看岗位原文</summary><pre className="raw-text">{job.raw_text}</pre></details><section className="panel"><div className="panel-heading"><h2>版本与分析历史</h2><span>{job.analyses.length} 次分析</span></div>{job.versions.map((version) => <p className="history-row" key={version.id}>JD v{version.version_number}<small>{formatChinaTime(version.created_at)}（北京时间） · {version.content_hash.slice(0, 12)}</small></p>)}{job.analyses.map((item) => <p className="history-row" key={item.id}>分析：{RECOMMENDATIONS[item.recommendation]} · {item.score}<small>{formatChinaTime(item.created_at)}（北京时间）</small></p>)}</section></section></>;
}
