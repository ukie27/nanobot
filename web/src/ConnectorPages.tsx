...
export function DataSourcesPage({ setupOnly = false }: { setupOnly?: boolean }) {
  ...
  return <>
    <header className="page-header"><div><p className="eyebrow">招聘信息来源</p><h1>招聘信息来源</h1><p>管理每日招聘信息与手动定向搜索。OpenCLI 只作为外部应用被调用。</p></div><span className={`health-pill ${nowcoder.data?.health_status === "healthy" ? "ok" : ""}`}>{connectorStatusLabel(nowcoder.data?.health_status, nowcoder.data?.last_error_code)}</span></header>
    <section className="notice"><strong>只读安全边界</strong><p>CareerConsole 通过外部 OpenCLI 读取牛客校招日程及可选的 BOSS 岗位信息，不提供打招呼、发消息、交换联系方式、邀请、收藏、订阅或自动投递。</p></section>
    ...
    <section className="panel">
      <div className="panel-heading"><div><p className="eyebrow">每日招聘</p><h2>牛客校招日程</h2></div><span>{nowcoder.data?.last_success_at ? `最近成功 ${formatChinaTime(nowcoder.data.last_success_at)}（北京时间）` : "尚未同步"}</span></div>
      ...
      <ConfigurationTestButton capability="nowcoder" label="测试牛客来源" />
    ...
    {!setupOnly && <>
    <section className="panel">
      <div className="panel-heading"><div><p className="eyebrow">手动定向搜索</p><h2>BOSS 直聘</h2></div><span>{query.data?.last_success_at ? `最近成功 ${formatChinaTime(query.data.last_success_at)}（北京时间）` : "尚未同步"}</span></div>
      ...
      <ConfigurationTestButton capability="boss" label="测试 BOSS 来源" />
    </section>
    <section className="panel table-wrap"><div className="panel-heading"><div><p className="eyebrow">同步记录</p><h2>BOSS 最近同步</h2></div></div><table><thead><tr><th>时间（北京时间）</th><th>触发</th><th>状态</th><th>发现</th><th>新增 / 更新 / 重复 / 隔离</th></tr></thead><tbody>{query.data?.runs.map(run => <tr key={run.id}><td>{formatChinaTime(run.started_at)}</td><td>{run.trigger_type}</td><td>{run.status}{run.error_code ? ` · ${run.error_code}` : ""}</td><td>{run.discovered_count}</td><td>{run.created_count} / {run.updated_count} / {run.duplicate_count} / {run.quarantined_count}</td></tr>)}</tbody></table></section>
