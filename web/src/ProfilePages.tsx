...
export function ProfilePage() {
  ...
  return <>
    <header className="page-header"><div><p className="eyebrow">可信职业档案</p><h1>{profile.data?.display_name ?? "我的职业档案"}</h1></div><span className="health-pill ok">已确认 {profile.data?.fact_counts.confirmed ?? 0}</span></header>
...
function ProfileBusinessMemory() {
  ...
  return <section className="profile-memory">
    ...
    <section className="panel"><div className="panel-heading"><div><p className="eyebrow">变化影响</p><h2>档案变化影响处理</h2></div><button className="secondary" onClick={() => impacts.mutate()} disabled={impacts.isPending}>处理待执行影响</button></div>{query.data?.impact_runs.map(item => <div className="history-row" key={item.id}>{item.scope} · {item.status}<small>影响 {item.affected_count} 项</small><details><summary>查看运行详情</summary><small>输入版本 {item.input_revision}{item.error_code ? ` · ${item.error_code}` : ""}</small></details></div>)}{!query.data?.impact_runs.length && <p>尚无影响处理记录。事实、偏好或已确认洞察变化后会创建持久化任务。</p>}</section>
...
function ManualFactForm() {
  ...
  return <section className="panel manual-fact"><div className="panel-heading"><div><p className="eyebrow">手动补充</p><h2>手动补充事实</h2></div><button className="secondary" onClick={() => setOpen(!open)}>{open ? "收起" : "添加"}</button></div>
...
export function ReviewPage() {
  ...
  return <><header className="page-header"><div><p className="eyebrow">人工确认</p><h1>事实审查</h1></div><button disabled={!selected.size || batch.isPending} onClick={() => batch.mutate()}>确认所选（{selected.size}）</button></header>
...
export function DocumentsPage() {
  ...
  return <><header className="page-header"><div><p className="eyebrow">档案依据</p><h1>资料来源</h1><p>导入简历和经历材料，提取出的事实需要人工确认后才会进入职业档案。</p></div></header>
    <section className="import-grid"><form className="panel upload-card"><p className="eyebrow">文件导入</p><h2>上传文件</h2><p>支持 TXT、Markdown、PDF、DOCX，最大 10 MB。</p><input type="file" accept=".txt,.md,.markdown,.pdf,.docx" onChange={(event) => { const file = event.target.files?.[0]; if (file) upload.mutate(file); }} />{upload.isPending && <small>正在解析…</small>}</form>
      <form className="panel" onSubmit={pasteSubmit}><p className="eyebrow">粘贴文本</p><h2>粘贴简历文本</h2><input name="name" placeholder="资料名称" required /><textarea name="text" rows={7} placeholder="粘贴简历内容" required /><button disabled={paste.isPending}>导入并提取候选事实</button></form></section>
    {result && <section className="notice success"><strong>{result}</strong><div className="form-actions"><Link className="download-button" to="/review">审查候选事实</Link></div></section>}{(upload.error || paste.error) && <section className="notice error">{upload.error?.message ?? paste.error?.message}</section>}
