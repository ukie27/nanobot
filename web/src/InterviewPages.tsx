...
import {
  cancelInterview, createInterview, generateInterviewPreparation, getApplication, getApplications, getImprovementItems,
...
export function InterviewCenterPage() {
  ...
  return <><header className="page-header"><div><p className="eyebrow">面试与复盘</p><h1>面试中心</h1></div><span className="health-pill ok">{interviews.data?.total ?? 0} 场</span></header>
    <section className="panel interview-create"><div><h2>安排一轮面试</h2><p>准备包只读取岗位冻结版本、实际投递材料、已确认事实和已确认历史弱点。</p></div><form className="form-grid" onSubmit={submit}><label>关联申请<select name="application_id" required defaultValue=""><option value="" disabled>选择申请</option>{applications.data?.items.map((item) => <option key={item.id} value={item.id}>{item.company} · {item.job_title}</option>)}</select></label><label>轮次<select name="round_type"><option value="phone">电话/HR 面</option><option value="technical">技术面</option><option value="case">案例面</option><option value="final">终面</option></select></label><label>时间（北京时间）<input name="scheduled_at" type="datetime-local" defaultValue={defaultInterviewTime()} required /></label><button disabled={create.isPending || applications.isLoading || !applications.data?.total}>{create.isPending ? "正在创建…" : "创建面试"}</button>{applications.isLoading ? <p className="disabled-reason">正在加载可关联申请…</p> : !applications.data?.total && <p className="disabled-reason">当前没有可关联的申请，请先在“申请总览”建立申请进度。</p>}{create.error && <p className="form-error">{create.error.message}</p>}</form></section>
...
export function InterviewDetailPage() {
  ...
  const application = useQuery({
    queryKey: ["application", query.data?.application_id],
    queryFn: () => getApplication(query.data?.application_id ?? ""),
    enabled: Boolean(query.data?.application_id),
  });
  ...
  const hasSubmissionSnapshot = Boolean(application.data?.material_snapshots.length);
  const preparationUnavailable = application.isLoading || !hasSubmissionSnapshot;
  return <><header className="page-header"><div><p className="eyebrow"><Link to="/interviews">面试中心</Link> / {ROUND_LABELS[interview.round_type]}</p><h1>{interview.company} · {interview.job_title}</h1><p>{formatChinaTime(interview.scheduled_at)}（北京时间）</p></div><button onClick={() => prepare.mutate()} disabled={prepare.isPending || preparationUnavailable}>{pack ? "重新生成冻结准备包" : "生成准备包"}</button></header>
    {!application.isLoading && !hasSubmissionSnapshot && <section className="notice warning"><strong>暂时不能生成面试准备包</strong><p>面试准备只读取确认投递时冻结的岗位和材料。请先在申请详情确认实际投递，并生成冻结材料快照。</p></section>}
    {application.error && <section className="notice error">{application.error.message}</section>}
