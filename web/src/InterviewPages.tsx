...
export function InterviewCenterPage() {
  ...
  return <><header className="page-header"><div><p className="eyebrow">INTERVIEW FEEDBACK LOOP</p><h1>面试中心</h1></div><span className="health-pill ok">{interviews.data?.total ?? 0} 场</span></header>
    <section className="panel interview-create"><div><h2>安排一轮面试</h2><p>准备包只读取岗位冻结版本、实际投递材料、已确认事实和已确认历史弱点。</p></div><form className="form-grid" onSubmit={submit}><label>关联申请<select name="application_id" required defaultValue=""><option value="" disabled>选择申请</option>{applications.data?.items.map((item) => <option key={item.id} value={item.id}>{item.company} · {item.job_title}</option>)}</select></label><label>轮次<select name="round_type"><option value="phone">电话/HR 面</option><option value="technical">技术面</option><option value="case">案例面</option><option value="final">终面</option></select></label><label>时间（北京时间）<input name="scheduled_at" type="datetime-local" defaultValue={isoToChinaInput()} required /></label><button disabled={create.isPending}>创建面试</button>{create.error && <p className="form-error">{create.error.message}</p>}</form></section>
    <section className="interview-grid">{interviews.data?.items.map((item) => <Link className="panel interview-card" to={`/interviews/${item.id}`} key={item.id}><span className="category-tag">{ROUND_LABELS[item.round_type]}</span><h2>{item.company} · {item.job_title}</h2><p>{formatChinaTime(item.scheduled_at)}（北京时间）</p><small>{item.preparation ? `准备包 v${item.preparation.version_number}` : "尚未生成准备包"} · {item.record ? "已复盘" : "待复盘"}</small></Link>)}</section>
    <section className="panel"><div className="panel-heading"><h2>待确认复盘建议</h2><span>{feedback.data?.items.filter((item) => item.status === "pending").length ?? 0}</span></div>{feedback.data?.items.map((item) => <article className="feedback-row" key={item.id}><div><strong>{item.category} · {item.description}</strong><blockquote>{item.evidence_text || "未提供回答摘要"}</blockquote></div>{item.status === "pending" ? <div className="fact-actions"><button onClick={() => resolve.mutate({ item, resolution: "confirmed" })}>确认沉淀</button><button className="danger" onClick={() => resolve.mutate({ item, resolution: "rejected" })}>拒绝</button></div> : <span>{item.status}</span>}</article>)}</section>
    <section className="panel"><div className="panel-heading"><h2>长期改进项</h2><span>{improvements.data?.items.filter((item) => item.status === "active").length ?? 0} 进行中</span></div>{improvements.data?.items.map((item) => <article className="feedback-row" key={item.id}><div><strong>{item.title} · 出现 {item.occurrence_count} 次</strong><p>{item.description}</p></div>{item.status === "active" ? <div className="fact-actions"><button onClick={() => update.mutate({ id: item.id, status: "completed" })}>完成</button><button className="secondary" onClick={() => update.mutate({ id: item.id, status: "dismissed" })}>忽略</button></div> : <span>{item.status}</span>}</article>)}</section></>;
}
...
export function InterviewDetailPage() {
  ...
  return <><header className="page-header"><div><p className="eyebrow"><Link to="/interviews">面试中心</Link> / {ROUND_LABELS[interview.round_type]}</p><h1>{interview.company} · {interview.job_title}</h1><p>{formatChinaTime(interview.scheduled_at)}（北京时间）</p></div><button onClick={() => prepare.mutate()} disabled={prepare.isPending}>{pack ? "重新生成冻结准备包" : "生成准备包"}</button></header>
    ...
    {pack && <><section className="metric-grid"><article><span>岗位要求</span><strong>{pack.requirements.length}</strong><small>冻结岗位版本</small></article><article><span>实际材料</span><strong>{pack.submitted_materials.length}</strong><small>投递快照</small></article><article><span>已确认事实</span><strong>{pack.confirmed_facts.length}</strong><small>不可虚构</small></article><article><span>历史改进</span><strong>{pack.historical_improvements.length}</strong><small>仅已确认</small></article></section><section className="preparation-grid"><PackList title="模拟问题" items={pack.questions} /><PackList title="面试反问" items={pack.candidate_questions} /><PackList title="检查清单" items={pack.checklist} /><PackList title="诚实 Gap Bridge" items={pack.gap_bridges.map((item) => `${item.requirement}：${item.bridge}`)} /></section><section className="panel"><div className="panel-heading"><h2>实际投递材料证据</h2><span>{pack.submitted_materials.length}</span></div>{pack.submitted_materials.map((item) => <details key={item.id}><summary>{item.title}</summary><p>{item.evidence_excerpt}</p><code>{item.content_hash}</code></details>)}</section></>}
